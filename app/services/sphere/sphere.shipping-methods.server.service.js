...
module.exports = function (app) {
    ...
    service.byCart = function (cartId, callback) {
    ...
    service.byLocation = function (country, state, currency, callback) {
        var path = '',
            params = [];
        ...
        app.logger.debug("Location path: %s",path);
    ...
    service.byLocationOneCurrency = function (country, state, currency, zonename, callback) {
        ...
        CommonService.byName('zones', zonename, function (err, zone) {
            ...
            if (err || zone == null || zone.length == 0) {
                ...
                service.byLocation(country, state, currency, function (err, result) {
                    if (err)
                        callback(err, null)
                    else {
